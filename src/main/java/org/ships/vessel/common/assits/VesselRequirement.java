    ...
    default void processRequirements(MovementContext context) throws MoveException {
        ...
        if (this.getMaxSize().isPresent() && (this.getMaxSize().get() < size)) {
            throw new MoveException(
                    new AbstractFailedMovement<>(this, MovementResult.OVER_SIZED, (size - this.getMaxSize().get())));
        }
        if (this.getMinSize() > size) {
            throw new MoveException(
                    new AbstractFailedMovement<>(this, MovementResult.UNDER_SIZED, (this.getMinSize() - size)));
        }
    }
